...
function blackhole(element) {
    ...
    const span = blackholeCenterHover.querySelector('span');

    // 原始文本内容
    const originalText = span.textContent;

    // 鼠标悬浮时的文本内容
    const hoverText = 'ENTER MY WEBSITE';

    blackholeCenterHover.addEventListener('mouseover', () => {
        span.textContent = hoverText;
    });

    blackholeCenterHover.addEventListener('mouseout', () => {
        span.textContent = originalText;
    });
    // Event listeners
    const centerHover = document.querySelector('.blackhole-centerHover');
    ...
    centerHover.addEventListener('click', function () {
        ...
        this.querySelector('span').classList.add('loading-span')
    ...
    function init() {
        ...
        context.fillRect(0, 0, cw, ch);
        for (let i = 0; i < 2000; i++) { // create 2000 stars
            new Star();
        }
        loop();
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updatePosition() {
        if (window.innerWidth < originalWidth) {
            // 窗口变小时，固定位置
            blackholeCenterHover.style.left = `${originalLeft}px`;
            blackholeCenterHover.style.top = `${originalTop}px`;
        } else {
            // 窗口变大时，保持居中
            blackholeCenterHover.style.left = '50%';
            blackholeCenterHover.style.top = '50%';
            blackholeCenterHover.style.marginTop = '-128px';
            blackholeCenterHover.style.marginLeft = '-128px';
        }
    }
    ...
    const enterButton = document.querySelector('.blackhole-centerHover');
